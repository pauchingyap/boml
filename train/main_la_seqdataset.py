...
def train(config, logger, run_spec, data_dir, seed=0):
    ...
    for task_idx, task in enumerate(config['dataset_ls'], 0):
        ...
        trainset = FewShotImageDataset(
            task_list=metatrain_dir_ls, supercls=config['supercls'], img_lvl=int(config['supercls']) + 1,
        ...
        if config['meta_train_eval_kwargs']['lapl_approx_reg']:
            ...
            if config['laplace_kwargs']['nll_supp_wrt_metaparam']:
                ...
                # save meta param mean and covs
                torch.save(lapl_approx.mpar_act_cov,
                           f=os.path.join(result_dir, 'mpar_act_covs.pt'.format(task_idx)))
                torch.save(lapl_approx.mpar_grad_cov,
                           f=os.path.join(result_dir, 'mpar_grad_covs.pt'.format(task_idx)))
                torch.save(lapl_approx.mpar_bn_fisher,
                           f=os.path.join(result_dir, 'mpar_bn_fishers.pt'.format(task_idx)))
