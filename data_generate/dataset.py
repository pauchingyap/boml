...
class FewShotImageDataset(Dataset):
    def __init__(self, task_list, supercls=True, img_lvl=1, transform=None, device=None, cuda_img=True,
                 verbose='dataset'):  # relabel=None,
        self.task_list = task_list
        self.supercls = supercls # true if data is tiered
        self.img_lvl = img_lvl # num of level below task dirs where imgs are located
        self.transform = transform
        self._relabel = None # tuple (colname, [val1, val2, ...])
        self.device = device
    ...
    def relbl_df(self):
        for ind, value in enumerate(self.relabel[1]):
            # row_mask = (self.df.index.isin(idx)) & list(self.df[self.relabel[0]] == value)
